...
int handle_write(void *data) {
    ...
    if (nwrited > 0) {
        ...
        //TODO: yolanda_msgx("handle_write for tcp connection %s", tcpConnection->name);
    }
...
void tcp_connection_shutdown(struct tcp_connection *tcpConnection) {
    if (shutdown(tcpConnection->channel->fd, SHUT_WR) < 0) {
        //TODO: yolanda_msgx("tcp_connection_shutdown failed, socket == %d", tcpConnection->channel->fd);
    }
}
